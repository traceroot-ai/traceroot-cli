...
describe("renderCsv", () => {
  ...
  it("quotes a header that needs it, like any other cell", () => {
    const out = renderCsv(["count, total"], [["1"]]);
    expect(out).toBe('"count, total"\n1\n');
  });

  it("writes a formula-looking cell verbatim", () => {
    const out = renderCsv(["name"], [["=SUM(A1:A2)"]]);
