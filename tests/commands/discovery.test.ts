...
describe("projects list", () => {
  ...
  it("lists unfiltered — the flow needs only the PROJECT ID column", async () => {
    const hooks: Record<string, unknown> = {};
    const { writers } = makeWriters();
    await runProjectsList({ client: client(hooks), json: true, writers });
    expect(hooks.lastProjectsParams).toBeUndefined();
  });
