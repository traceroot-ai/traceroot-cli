...
describe("runStatus with an API key (human)", () => {
  ...
  it("shows only the hint when the key has no name", async () => {
    const { writers, out } = makeWriters();
    ...
    // apiKeyLabel's null-name fallback: hint only, never "(unknown)" when a
    // hint exists, and no stray "null" text.
    expect(out.data).toContain("tr_***1234");
    expect(out.data).not.toContain("ci-key");
    expect(out.data).not.toContain("(unknown)");
    expect(out.data).not.toContain("null tr_");
  });
...
describe("runStatus with a session credential (human)", () => {
  ...
  it("shows the default project and its source", async () => {
    ...
    // The SOURCE renders too (sessionAuth's projectId comes from the env).
    expect(out.data).toContain("environment");
  });
  ...
  it("re-throws an AUTH failure from the workspace read — status must not report a dead session as healthy", async () => {
    const { writers, out } = makeWriters();
    const authFailure = new CliError("jwt rejected", ExitCode.auth);
    await expect(
      runStatus({
        ctx: makeContext(sessionAuth(), false),
        client: fakeClient({
          listWorkspaces: () => Promise.reject(authFailure),
        }),
        writers,
        getAccessToken: () => Promise.resolve(jwtWith({ email: "kai@example.com" })),
      }),
