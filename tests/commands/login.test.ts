...
describe("runLogin device flow (default)", () => {
  ...
  it("overwrites a stale config host_url when logging in to the default host", async () => {
    const h = makeHarness();
    await runLogin(
      baseDeps(h, {
        resolvedHost: DEFAULT_HOST,
        hostSource: "flag",
        readConfig: () => ({ host_url: "https://staging.example" }),
      }),
    );
    // The session is stored under DEFAULT_HOST; the config must follow, or the
    // next command resolves the stale staging host and can't find it.
    expect(h.credentialWrites[0]?.host).toBe(DEFAULT_HOST);
    expect(h.writeConfigCalls).toEqual([{ host_url: DEFAULT_HOST }]);
  });
...
describe("runLogin already logged in", () => {
  ...
  it("stored session, non-interactive + dead: re-runs the device flow instead of no-op", async () => {
    const h = makeHarness();
    await runLogin(
      baseDeps(h, {
        credential: { kind: "session", value: SESSION, source: "credentials-file" },
        ...
        // The mint probe fails with an auth error → the session is revoked.
        mintAccessToken: () => Promise.reject(new CliError("session expired", ExitCode.auth)),
      }),
    );
    // The prescribed recovery actually recovers.
    expect(h.deviceFlowRuns).toHaveLength(1);
    expect(h.credentialWrites).toHaveLength(1);
  });

  it("stored session, non-interactive + alive: warns and does not re-auth", async () => {
    const h = makeHarness();
    await runLogin(
      baseDeps(h, {
        credential: { kind: "session", value: SESSION, source: "credentials-file" },
        ...
        isInteractive: false,
        readCredentialEntry: () => ({ session_token: SESSION, email: "kai@example.com" }),
        // The mint probe succeeds → session is alive.
        mintAccessToken: () => Promise.resolve(jwtWith({ email: "kai@example.com" })),
      }),
    );
    expect(h.deviceFlowRuns).toHaveLength(0);
    expect(h.err.data).toContain("Already logged in");
