...
/** An API key is `tr_`/`tr-` prefixed; a session token never is. */
function looksLikeApiKey(value: string): boolean {
  return /^tr[_-]/.test(value);
}
...
function firstCredential(candidates: CredentialCandidate[]): ResolvedCredential {
  for (const candidate of candidates) {
    ...
    // A `tr_`-shaped value in a session slot (e.g. an API key mistakenly put in
    // TRACEROOT_TOKEN) is treated as an API key, not POSTed to the mint route as
    // a session token — which would 401 with a confusing error.
    const kind =
      candidate.kind === "session" && looksLikeApiKey(value) ? "api-key" : candidate.kind;
    return { kind, value, source: candidate.source };
  }
...
export function resolveAuth(options: ResolveAuthOptions = {}): ResolvedAuth {
  ...
  const authHost = firstPresent(
    [
      ...
      // The stored entry's auth_host belongs to the STORED session; honor it
      // only when that session is the winning credential. A TRACEROOT_TOKEN
      // that outranked it may come from a different auth server, and minting
      // there would hand the token to the wrong host.
      {
        value: credential.source === "credentials-file" ? storedEntry?.auth_host : undefined,
        source: "credentials-file",
      },
      // A repo-shared .env may pin the dev auth host (mirroring how it can pin
      // TRACEROOT_HOST_URL); unlike a session token, an auth host is not a
      // personal secret, so the auto-.env is allowed to supply it.
      { value: autoEnv.TRACEROOT_AUTH_URL, source: "auto-env-file" },
