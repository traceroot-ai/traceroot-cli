...
export const tracesList: Enhancer = {
  ...
  flags(cmd: Command): void {
    // Deliberately only the basic time/limit filters. Field filters (--name,
    // --user-id, --search-query, --filters, --include-evaluations) were removed:
    // filtering is moving to the upcoming SQL query surface instead of growing
    // a per-field flag vocabulary that would be deprecated on arrival.
